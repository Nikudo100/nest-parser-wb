  ...
  export default function Home() {
    ...
    const [selectedOurProductId, setSelectedOurProductId] = useState<number | null>(null);

    const addCompetitor = async (competitorId: number) => {
      if (!selectedOurProductId) {
        alert("Сначала выберите наш товар (is_our_product)");
        return;
      }

      await fetch(`${process.env.NEXT_PUBLIC_SERVER_HOST}/parser/product/${selectedOurProductId}/add-competitors?competitorIds=${competitorId}`, {
        method: 'POST',
      });
    ...
    return (
        <div className="container mx-auto max-w-7xl">
          ...
          {selectedOurProductId && (
            <div className="p-3 mb-4 text-green-800 bg-green-100 rounded border border-green-300">
              Вы выбрали товар ID: <strong>{selectedOurProductId}</strong> как «наш». Теперь можно добавлять к нему конкурентов.
            </div>
          )}
                    ...
                    <td className="p-2 text-center border">
                      {product.is_our_product ? (
                        <div className="space-y-3">
                          <button
                            className={`px-4 py-2 rounded-lg shadow-md transition-all duration-200 ${
                              selectedOurProductId === product.id
                            ...
                            onClick={() => setSelectedOurProductId(product.id)}
                          >
                            {selectedOurProductId === product.id ? '✓ Выбран' : '🏷 Наш товар'}
                          </button>
                          <br/>
                          <br/>
                          <a
                        ...
                        <button
                          ...
                          onClick={() => addCompetitor(product.id)}
                        >
